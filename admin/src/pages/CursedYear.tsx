...
const CursedYear: React.FC = () => {
  const [items, setItems] = useState<CursedYearItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<CursedYearItem | null>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [formData, setFormData] = useState({
    name: "",
    description: "",
    image: "",
    categoryId: "b596de94-876f-42f5-bfd0-09007aa11bfd",
  });
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        "http://localhost:5000/api/category/b596de94-876f-42f5-bfd0-09007aa11bfd"
      );
  ...
  const handleEdit = (item: CursedYearItem) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/cursedyears/${selectedItem.id}`
      );
  ...
  const handleSubmit = async () => {
    try {
      if (isEditing && selectedItem) {
        ...
        await axios.post("http://localhost:5000/api/cursedyears", formData);
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="space-y-4 p-4 bg-[#1A0040] text-white rounded-lg shadow-lg w-[90vw] max-w-md">
          <h2 className="text-xl font-bold text-center text-[#D2B589] font-prompt">
            {isEditing ? "แก้ไขปีชง" : "เพิ่มปีชง"}
          </h2>
          <input
            type="text"
            placeholder="ชื่อปีชง"
            value={formData.name}

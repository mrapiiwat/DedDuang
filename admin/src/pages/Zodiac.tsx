...
const Zodiac: React.FC = () => {
  const [items, setItems] = useState<AuspiciousItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<AuspiciousItem | null>(null);
  ...
  const handleEdit = (item: AuspiciousItem) => {
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="space-y-4 p-4 bg-[#1A0040] text-white rounded-lg shadow-lg w-[90vw] max-w-md">
          <h2 className="text-xl font-bold text-center text-[#D2B589] font-prompt">
            {isEditing ? "แก้ไขฤกษ์ราศี" : "เพิ่มฤกษ์ราศี"}
          </h2>
          <input
            type="text"
            placeholder="ชื่อราศี"
            value={formData.name}
